...
export default {
	async fetch(request, env, ctx) {
		try {
			...
			// 新增PROXYIPAPI和SOCKS5API处理
			if (env.PROXYIPAPI) {
				const proxyIPAPIList = await 整理(env.PROXYIPAPI);
				const newProxyIPs = await 整理优选列表(proxyIPAPIList);
				proxyIPs = proxyIPs.concat(newProxyIPs);
				proxyIP = proxyIPs[Math.floor(Math.random() * proxyIPs.length)];
			}
			
			if (env.SOCKS5API) {
				const socks5APIList = await 整理(env.SOCKS5API);
				const newSocks5s = await 整理优选列表(socks5APIList);
				socks5s = socks5s.concat(newSocks5s);
				socks5Address = socks5s[Math.floor(Math.random() * socks5s.length)];
				socks5Address = socks5Address.split('//')[1] || socks5Address;
			}
